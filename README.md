...
- Camera mode behavior:
  ...
  - **Pose timer (before snapshot):** fixed **10 s** on the **live camera** before the snapshot (large **“Get ready — snapshot in N”** overlay; same for **Capture again**). This is **not** the rear **auto-print** countdown (default **10 s after** the photo). Optional **`PHOTOBOOTH_CAPTURE_COUNTDOWN_SEC`** in **`prepare-www`** overrides or sets **0** to disable the pose wait (see `.env.example`). **Rebuild** mobile after changing `apps/web`.
...
**Second printer / external utility (drop zone):** you can **also** copy each new final into another folder that your other tool watches. In **`.env.standalone`** set **`PHOTOBOOTH_COPY_FINAL_TO_DROPZONE=1`** and **`PHOTOBOOTH_DROPZONE_DIR`** to an absolute or repo-relative path. Example (Google Drive on Windows, spaces in path): **`PHOTOBOOTH_DROPZONE_DIR="G:/My Drive/NandS/Input"`** — keep **Google Drive for desktop running** and the **`G:`** drive **mounted** when the booth runs (a disconnected or “files on demand” folder can cause copies to fail silently from older builds). The API saves the same base name as the local final (adds a timestamp suffix only if that name already exists). Check **`/health`** for **`dropzone_copy_enabled`** and **`dropzone_dir`**. On **API startup**, the console always prints a short **env snapshot** (raw `PHOTOBOOTH_*` values + resolved dropzone path) so you can confirm the **uvicorn** process actually loaded **`.env.standalone`**. On each final, it prints **`[photo-booth] dropzone: OK → …`** or **`FAILED`** with a full traceback after retries (it retries with `copy2` then `copyfile`).
